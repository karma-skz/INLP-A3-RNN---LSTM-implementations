...
from src.common.layers import ManualRNNDecryptor
...
def _build_model(model_config: dict, source_vocab_size: int, target_vocab_size: int) -> ManualRNNDecryptor:
    return ManualRNNDecryptor(
...
def main(config_path: str, mode: str = "evaluate"):
    ...
    if wandb_cfg.get("enabled", False):
        wandb_run = init_wandb(
            ...
            config=config,
            name=wandb_cfg.get("run_name", "task1_rnn"),
    ...
    if wandb_run is not None:
        save_wandb_run_info(wandb_run, os.path.join(config["paths"]["results_dir"], "task1_rnn_wandb.json"))
    ...
    if mode in {"train", "both"}:
        ...
        for epoch in range(1, config["train"]["epochs"] + 1):
            train_loss = run_epoch(model, train_loader, optimizer, pad_idx, device, grad_clip=grad_clip)
            val_loss = run_epoch(model, val_loader, None, pad_idx, device)
            print(f"[task1_rnn] epoch={epoch} train_loss={train_loss:.4f} val_loss={val_loss:.4f}")
            if wandb_run is not None:
    ...
    if wandb_run is not None:
        log_wandb(metrics)
    out_file = os.path.join(config["paths"]["results_dir"], "task1_rnn.txt")
    write_results(out_file, metrics, decrypted)
    print(f"[task1_rnn] wrote results: {out_file}")
    if wandb_run is not None:
        finish_wandb()
