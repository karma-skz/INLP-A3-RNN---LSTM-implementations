...
from src.common.layers import SimpleSSM
from src.task2.common import build_nwp_dataloaders, run_nwp_epoch
...
def main(config_path: str, mode: str = "evaluate"):
    ...
    if wandb_cfg.get("enabled", False):
        wandb_run = init_wandb(
            ...
            config=config,
            name=wandb_cfg.get("run_name", "task2_ssm"),
    ...
    if wandb_run is not None:
        save_wandb_run_info(wandb_run, os.path.join(config["paths"]["results_dir"], "task2_ssm_wandb.json"))

    if mode in {"train", "both"}:
        train_loader, val_loader, vocab = build_nwp_dataloaders(config)
        ...
        for epoch in range(1, config["train"]["epochs"] + 1):
            train_loss, train_ppl = run_nwp_epoch(model, train_loader, optimizer, device, grad_clip=grad_clip)
            val_loss, val_ppl = run_nwp_epoch(model, val_loader, None, device)
            print(
                f"[task2_ssm] epoch={epoch} train_loss={train_loss:.4f} train_ppl={train_ppl:.2f} "
    ...
    _, val_loader, _ = build_nwp_dataloaders(config)
    val_loss, val_ppl = run_nwp_epoch(model, val_loader, None, device)
    out = {"task": "task2_ssm", "val_loss": val_loss, "perplexity": val_ppl}
    if wandb_run is not None:
        log_wandb({"final_val_loss": val_loss, "final_val_perplexity": val_ppl})
    out_file = os.path.join(config["paths"]["results_dir"], "task2_ssm.txt")
    with open(out_file, "w", encoding="utf-8") as handle:
        handle.write(json.dumps(out, indent=2))
    print(f"[task2_ssm] wrote results: {out_file}")
    if wandb_run is not None:
        finish_wandb()
